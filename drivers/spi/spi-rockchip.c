...
static void rockchip_spi_dma_rxcb(void *data)
{
	...
	struct rockchip_spi *rs = spi_master_get_devdata(master);
	int state = atomic_read(&rs->state) & ~RXDMA;
	atomic_xchg(&rs->state, state);
...
static void rockchip_spi_dma_txcb(void *data)
{
	struct spi_master *master = data;
	struct rockchip_spi *rs = spi_master_get_devdata(master);
	int state = atomic_read(&rs->state) & ~TXDMA;
	atomic_xchg(&rs->state, state);
...
static size_t __maybe_unused rockchip_spi_max_transfer_size(struct spi_device *spi)
{
...
static int rockchip_spi_probe(struct platform_device *pdev)
{
	...
	master->dma_tx = dma_request_slave_channel(rs->dev, "tx");
	...
	master->dma_rx = dma_request_slave_channel(rs->dev, "rx");
